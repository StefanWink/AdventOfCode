...
public class JokerHandComparer : IComparer<Hand>
{
    private readonly JokerCardComparer cardComparer = new();
    ...
    private static HandType GetHandType(Hand hand)
    {
        ...
        int jokers = hand.Cards.Count(c => c == joker);
        ...
        if (grouping[0].Count() + jokers == 5)
            return HandType.FiveOfAKind;

        if (grouping[0].Count() + jokers == 4)
            return HandType.FourOfAKind;

        if (grouping[0].Count() + jokers == 3)
            return grouping[1].Count() == 2
                ? HandType.FullHouse
                : HandType.ThreeOfAKind;

        if (grouping[0].Count() + jokers == 2)
            return grouping[1].Count() == 2
                ? HandType.TwoPair
                : HandType.OnePair;
